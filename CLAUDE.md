...
**Stages 0-7 are complete**; Stage 8 (concatenated members and trailing bytes) is next. miniz 3.1.2 is vendored under `src/vendor/miniz/`, `inst/include/zukomp.h` carries the ABI vocabulary plus the codec vtable and registry functions, and the registry ships with the `identity` codec. `komp_codecs()` and `komp_codec_available()` are the only exported R functions. `devtools::check(cran = TRUE)` is 0/0/0.

Four codecs are registered — `identity`, `deflate-raw`, `zlib`, `gzip` — and both limits are enforced and tested.

**Known gap until Stage 8:** a concatenated multi-member gzip stream decodes only its first member and returns *without error*. `tests/testthat/fixtures/gzip/py_multimember_ascii.bin` exists for this and is excluded from the Stage 7 interop test by `members == 1L`. Stage 8 adds the member loop and `zukomp_trailing_bytes`. **There is still no whole-buffer R API**: `komp_compress()`/`komp_decompress()` arrive at Stage 9, so tests drive codecs through `zu_test_stream()`. `src/zu_miniz.c` is temporary Stage 1 scaffolding behind `zukomp:::zu_miniz_version()` that Stage 9's `komp_info()` replaces.
