...
Deliberately outside testthat, in CI jobs: sanitizers, valgrind, LTO, gctorture and `rchk` (`native-checks.yaml`, which calls the shared reusable workflows from `pedrobtz/r-actions@v1`), the consumer package (`consumer.yaml`), fuzzing and MSan (`fuzz.yaml`), external-decoder interop (`tools/check-interop.sh`), the standalone-header and consumer-build probes (`abi.yaml`), and the vendor guard (`vendor.yaml`). Benchmarks (`bench/`) are still phase 2. `native-checks.yaml` keeps one extra job, `sanitizers-exhaustive`, beside the shared one: the reusable workflow takes no inputs, so `ZUKOMP_SLOW_TESTS` cannot be passed into it (leaving only the sampled sweeps), and it *halts* on a UBSan finding (`UBSAN_OPTIONS=…:halt_on_error=1` plus `-fno-sanitize-recover=undefined`) rather than only printing one — both Stage 14 fuzz findings were UBSan findings, and a job that prints them and stays green would have missed both. Its steps pipe into `tee`, so the job sets `shell: bash` for pipefail; without it the step's status is `tee`'s and a failing install or suite is green. Delete that job if the shared workflow ever gains an env passthrough and halts on UBSan.
...
- **`sanitizers-exhaustive` is UBSan only, on purpose.** ASan instruments a package `.so` fine, but that `.so` is `dlopen`'d into an R that is not itself instrumented, which needs `-shared-libasan`, an `LD_PRELOAD` of the runtime, and `detect_leaks=0` (R does not free on exit). Real ASan coverage of the C core is in `fuzz/build.sh --standalone`, where it links into an actual executable; the R glue layer is covered by valgrind, `rchk` and gctorture. UBSan still has to bring its own runtime: clang links none into a `-shared` output, so the job needs `-shared-libsan` and `-Wl,-rpath,$(clang -print-runtime-dir)` on top of `LDFLAGS`, or the `.so` fails to load on `__ubsan_handle_*` the same way it did on `__asan_*`. What makes UBSan the cheap one is only that its shared runtime can arrive by `DT_NEEDED` at `dlopen` time, where ASan must be first in the process and so needs an `LD_PRELOAD` on R itself. The job verifies instrumentation on the *installed* `.so` with `nm`, not by grepping the build log — the log records the command that was meant to produce the object, which is exactly what was wrong before.
