...
extras.** When a choice would make DEFLATE special, it is the wrong
choice.
...
so it never advertises a symbol that will not link — keep that rule for
phase 2. Everything described below is shipped code.

## The design docs are the spec

[.agents/design-zukomp.md](https://pedrobtz.github.io/zukomp/.agents/design-zukomp.md)
and
[.agents/ROADMAP.md](https://pedrobtz.github.io/zukomp/.agents/ROADMAP.md)
are not background reading — they are the binding specification, down to
struct field order, enum values, symbol prefixes, and per-stage test
code. **Read the relevant section before writing code**, and if
...
because the clock check cannot reach the network. Suppress it to see the
real result:
...
Exhaustive sweeps and large-buffer tests are gated behind
...
Third-party code under `src/vendor/` is **never edited in place**. The
whole tree is reproducible from `tools/vendor/manifest.tsv`:
...
`verify` fails on a modified vendored file, an unrecorded new one, and
any drift between `src/Makevars`’s `-D` flags and the manifest’s
...
- The define set is **seven**, not the five design §12 originally
  listed. `MINIZ_NO_PNG_APIS` is *ours*: upstream guards the PNG writer
  only by `MINIZ_NO_DEFLATE_APIS`, which we need, so
  `tools/patches/miniz/0001-guard-png-writer.patch` adds the guard.
  Without it the PNG entry points are exported and `test-abi.R` fails.
- `MZ_ASSERT` expanded to `assert` at 26 sites reachable from malformed
  input, and upstream defines it unconditionally so it could not be
  overridden from the command line — what it did was decided entirely by
  `NDEBUG`. R supplies `-DNDEBUG`, so normal and CRAN builds always
  compiled it away, but a `-UNDEBUG` build (`devtools`’ debug install)
  could `abort()` the R session instead of raising a condition.
  `tools/patches/miniz/0003-guard-mz-assert.patch` wraps the definition
  in `#ifndef` and adds `MINIZ_NO_ASSERT`, which `src/Makevars` sets.
  This removes no checking a release build was doing — those sites were
  already no-ops — it stops the debug build from diverging from the one
  that ships. `komp_info()$build_flags` reports the define, and
...
**Codecs are vtables behind an enum, discovered at runtime.**
`zu_codec_vtable` (design §10) supplies encoder/decoder quartets, level
range, magic bytes or a sniff predicate, and metadata. Adding a codec is
a new enum value plus a new vtable — no header change, no ABI bump.
`zu_register_codec()` may only be called during `R_init_*`, before any
stream exists.

**Limits live in the core driver, never in a codec.** `max_output` and
`max_ratio` are enforced by `src/zu_stream.c`, which sees every byte
through the `zu_buffer` cursors. This is why a third-party codec nobody
here reviewed still cannot bypass the output cap — Stage 12’s
consumer-package test exists to prove exactly that.

**One axis, not two.** Wrapper variants are distinct codec identities
(`deflate-raw`, `zlib`, `gzip` are three codecs, not one algorithm with
a `format` argument). Levels are **codec-native** and validated against
the codec’s advertised range; no cross-codec numeric equivalence is
claimed.

**Whole-buffer functions drive the streaming engine — there is no second
code path.** `zu_int_run_whole()` in
[src/zu_whole.c](https://pedrobtz.github.io/zukomp/src/zu_whole.c) is
that one loop;
[`komp_compress()`](https://pedrobtz.github.io/zukomp/reference/komp_compress.md),
[`komp_decompress()`](https://pedrobtz.github.io/zukomp/reference/komp_decompress.md)
and `zu_test_stream()` all call it. This is why the chunk-boundary
sweeps are worth anything: a separate whole-buffer loop would mean every
sweep tested code no user runs. It also carries the design §13
obligations in one place — the output sink owned by an external pointer
...
and a `$(wildcard)` would force `SystemRequirements: GNU make`, which
design §12 forbids. A file that is not in `OBJECTS` is silently not
built.
...
Never exported under any circumstances: `deflate`, `inflate`,
...
- **`inst/include/zukomp.h` must compile standalone as C99** against
  only `<stddef.h>`/`<stdint.h>`. No `R.h`, no `SEXP`, no miniz type or
  symbol, no DEFLATE vocabulary (`zu_encoder`/`zu_decoder`, never
  `deflater`/`inflater`). R-specific resolution belongs in `zukomp-r.h`.
  Enforced two ways, and they use the same comment-stripped rule so keep
  them in step: the `abi.yaml` workflow compiles the header standalone
  under `-Werror` (C99 and C++), and `test-abi.R` greps the *installed*
  copy via `installed_header_code()`. The rule targets declarations, not
  prose — a comment may name miniz, a declaration may not.
- **`MINIZ_NO_ZLIB_COMPATIBLE_NAMES` is not optional.** Without it
  `miniz.h` `#define`s `compress`, `crc32`, `adler32` and friends over
  every translation unit that includes it — including ones that also see
  R’s headers, since R links its own zlib.
- **Anything holding heap state across a longjmp must be owned by R.**
  `Rf_error()` *and* `R_CheckUserInterrupt()` both jump straight past
  any `free()` below them. The stream handle in `zu_int_run_whole()` is
  therefore held by an external pointer with
  `R_RegisterCFinalizerEx(..., TRUE)` for the duration of the loop, and
  freed eagerly (pointer cleared first) on the normal path. This is not
  theoretical: the first version of that loop called
  `R_CheckUserInterrupt()` with the handles in bare locals, leaking one
  stream per interrupted decompression.
- **No `Rf_error()` inside the codec loop.** C returns `zu_status`; only
  the outermost `.Call` raises. `Rf_error()` and
  `R_CheckUserInterrupt()` both longjmp past `free()`. Growing buffers
  use `R_alloc` with `vmaxget`/`vmaxset`, or an external pointer with
  `R_RegisterCFinalizerEx(ptr, fin, TRUE)`.
- **The output sink is `malloc`/`realloc` under an external pointer, not
  `R_alloc`.** `R_alloc` cannot resize, so growing meant allocating a
  new block and copying, and every superseded block stayed on the `vmax`
  stack until the enclosing `.Call` returned — with doubling, several
  were live at once and decoding 64 MB peaked at ~199 MB. `realloc`
  releases as it goes, and `R_SetExternalPtrAddr` follows the buffer on
  every move. Two traps. The external pointer must be updated on *every*
  growth, or the finalizer frees a stale block and leaks the live one.
  And the gc-drift leak tests in `test-memory.R` watch R’s `Vcells`,
  which cannot see a `malloc`’d buffer at all — so
  `zu_int_outbuf_live_count()`, surfaced as `zu_test_outbuf_live()`, is
  what actually proves the sink is freed on the error and interrupt
  paths. Note the encode path is deliberately *not* pre-sized from
  `zu_compress_bound()`: the bound is roughly the input size, while the
  growth path now peaks at roughly the output size, so pre-sizing would
  lose badly on exactly the compressible data compression is for.
- **“The trailer ended” is not “the stream ended”.** A gzip member may
  be followed by another, so the decoder goes to `ST_MEMBER_END` and
  decides. Whether more input exists is only knowable at `ZU_FINISH` —
  before that, an empty buffer means “not yet”, not “no more”. A
...
- **`ZU_FINISH` arrives with the final bytes, not on a later empty
  call.** `last` used to also require the buffer to be spent, and the
  refill resets `src_pos` to 0, so `ZU_FINISH` only ever reached a codec
  alongside an empty buffer and no codec could tell “these are the last
  bytes” from “here are some bytes” — which is why gzip’s member probe
  had to take a trailing `0x1F` speculatively. Changing it made one
  previously-unreachable path reachable, and that path was wrong: the
  DEFLATE body’s “told there is no more input, yet the stream has not
  ended” returned `ZU_ERR_TRUNCATED`, contradicting design §7’s own rule
  that only a short *wrapper field* is truncation and a short body is
  `ZU_ERR_INVALID_DATA`. `test-truncation.R`’s position-by-position
  table is what caught it — it fails in exactly two places if you make
  this change without the classification fix.
- **A following gzip member is confirmed on both magic bytes, never on
  `1f` alone.** Deciding on the first byte committed the decoder to
  parsing a member, so any tail starting with `0x1f` became
  `zukomp_invalid_data` instead of a trailing-data decision — and with
  `ZU_DEC_REJECT_TRAILING` off, bytes the caller had elected to ignore
  failed the decode anyway. The awkward part is that the two bytes can
  arrive in separate `process()` calls and the driver only refills once
  the codec has consumed everything, so “wait without consuming”
  deadlocks. Hence: decide without consuming whenever both bytes are
  visible (which keeps `src_pos` exact), and otherwise hold the `0x1f`
  in `probe_magic` and decide on the next call. The one case `src_pos`
  cannot express is input ending on a held `0x1f`, where the codec
  applies `ZU_DEC_REJECT_TRAILING` itself — without that, the same
  trailing byte was reported at a large chunk size and silently
  swallowed at `in_chunk = 1`. **A held `0x1f` that is then disproved is
  counted as consumed**, so the reported cursor runs one byte past the
  true stream end. Only when the two magic bytes land in *different*
  buffers — a lone trailing `0x1f` is exact now that `ZU_FINISH` arrives
  with the final bytes. The classification is identical at every chunk
  size; the count is not, and `test-members.R` pins the remainder as a
  known deviation rather than accepting it. Closing it needs a pushback
  in the core, since `src_pos` belongs to a call that has already
  returned.
- **Limits must be whole numbers, and that is a security property.**
  `max_output` and `max_ratio` are narrowed to integer types (`uint64_t`
  and `uint32_t`), which truncate toward zero, and native `0` means *no
  limit* — so a limit anywhere in `(0, 1)` used to truncate to
  “unlimited” and disable the decompression-bomb guard entirely.
  `zu_check_limit()` rejects any finite fractional value, and
  `zu_int_u64_from_real()` carries a matching backstop so the C boundary
  cannot accept one either. `NULL`, `0` and `Inf` remain the spellings
  for unlimited.
- **The
  [`komp_codecs()`](https://pedrobtz.github.io/zukomp/reference/komp_codecs.md)
  cache keys on registry mutation, never on the table’s shape.**
  `zu_int_registry_generation()` counts successful registrations; the R
  closure rebuilds when it changes. Keying on the displayed row count is
  wrong because a satellite implementing a *declared* codec flips a row
  from unavailable to available without adding one — so a table warmed
  before that package loaded stayed stale, and
...
- **Registration’s name and content-coding checks apply to *every*
  ...
  claim another declared codec’s HTTP token, and
  `zu_codec_from_content_encoding()` scans the declared table first, so
  the token kept resolving to the wrong — possibly unavailable — codec.
  The comparison is “same name or token, different identity”.
- **Registration validates every key a codec is looked up by, not just
  the numeric id.** The registry is process-global, append-only and has
  no removal API, so one malformed satellite poisons discovery for the
  whole session. A vendor id registering the name `"gzip"` gave
  [`komp_codecs()`](https://pedrobtz.github.io/zukomp/reference/komp_codecs.md)
  two rows with that id, and R’s scalar `if (!row$available)` then got a
  length-two logical and errored — while C lookup still preferred the
  built-in, so the two layers resolved the same name differently.
  `zu_register_codec()` now requires a declared id to carry its declared
  name, an undeclared id to sit at or above `ZU_CODEC_VENDOR_BASE`, and
  both names and content-coding tokens to be unique — including against
  declared-but-unavailable names, which are reserved for a satellite to
  claim *with the declared id*.
- **Trailing bytes are policy, and policy lives in the options.** The
  ...
  `ZU_DEC_CONCAT_MEMBERS` likewise gates member continuation. Both
  default to on in `zu_test_stream()`, matching whole-buffer semantics.
- **The gzip header parser is fed one byte at a time**
  (`src/zu_gzip.c`). A header field can straddle any number of
  `process()` calls — the sweeps run at one byte per call — and
  byte-at-a-time is the only shape where that is obviously correct
  rather than merely tested. It is kept in its own file to stay
  separately fuzzable in Stage 14. FNAME/FCOMMENT bytes are discarded,
  never buffered, so a hostile multi-megabyte filename costs time and
  nothing else.
- **zukomp owns the zlib wrapper; miniz only does raw DEFLATE.**
  `codec_deflate.c` drives miniz at `window_bits = -15` and implements
  RFC 1950’s two-byte header and Adler-32 trailer itself. This is not
  incidental: `mz_inflate()` reports a corrupt Adler-32 and corrupt
  compressed data both as `MZ_DATA_ERROR`, and design §24 criterion 6
  requires checksum failures to surface distinctly as
  `zukomp_checksum_error`. Stage 7’s gzip wrapper reuses this
  `ST_HEADER`/`ST_BODY`/`ST_TRAILER` structure. Every wrapper byte is
  streamed one at a time if that is all the room there is, because the
  chunk sweeps run at `in_chunk = out_chunk = 1`.
- **`zu_test_stream()` is the main test lever.** It drives the C driver
...
- **Limits shrink the codec’s window; they are not a post-hoc check.**
  ...
  landing *exactly* on the cap is not an error (the stream still needs
  one more call to observe `ZU_FINISH`), and it is the codec asking for
  **more** room that distinguishes “finished at the cap” from “would
  have exceeded it”. `max_ratio` is checked after the fact instead,
  since `max_output` already bounds memory.
- **Never allocate based on a size claimed by the input.** gzip’s ISIZE
  is validated against actual output, never used to size a buffer. All
  buffer arithmetic goes through checked `zu_add`/`zu_mul`/`zu_grow` —
  never a bare `size *= 2`.
- **`codec = "auto"` never falls back to a headerless codec.**
  `zu_sniff()` tries fixed magic first (longest first, so a short magic
  cannot shadow a longer one), then predicate sniffers, then errors
  `zukomp_undetectable_codec`. A codec with neither magic nor sniffer is
  never detected — that is what keeps `auto` off raw DEFLATE, identity
  and brotli, where guessing wrong means silently returning the wrong
  bytes. zlib’s predicate has a measured ~0.1% false-positive rate on
  random bytes (matching the theoretical 1/(16·2·31)), which
  `test-detect.R` documents rather than hides.
- **Codec enum values are permanent.** A codec compiled out keeps its
  number and reports unavailable.
- **The ABI reaches consumers through a registered C-callable table,
...
- **The abstract level names are declared by the codec, never derived
  from its range.**
  [`komp_codecs()`](https://pedrobtz.github.io/zukomp/reference/komp_codecs.md)
  publishes `level_fast` and `level_best` beside
  `level_min`/`level_max`/`level_default`, and `"fast"`/`"best"` resolve
  through those two. Deriving them from the range endpoints is wrong in
  both directions: DEFLATE’s `level_min` is 0, which is stored blocks,
  so `"fast"` would return output *larger* than the input; and LZ4’s
  level is an acceleration factor where higher means faster, so its
  `"fast"` sits at `level_max`. `"default"` is
  `NULL`/`ZU_LEVEL_DEFAULT`, and a codec with no level axis accepts all
  three names and gets its default for each — rejecting two of them
  would mean codec-agnostic code still has to special-case levels, which
  is the whole thing the names exist to avoid.
- **Every `struct_size` check requires only the prefix the core
  dereferences, and every opts struct is copied bounded.**
  `zu_encoder_opts`/`zu_decoder_opts` have `ZU_*_OPTS_REQUIRED_SIZE` for
  the same reason the vtable does — a check for the full current
  `sizeof` makes each appended field a breaking change for consumers
  built against an older header. The half that is easy to miss: once a
  shorter struct is accepted, `*dst = *src` reads past the end of the
  caller’s memory, so `zu_int_copy_opts()` copies
  `min(struct_size, sizeof)`, zero-fills the rest, and stamps our own
  `struct_size`. The normalised copy is what the codec receives, so a
  codec never sees a short struct either.
- **`zu_codec_vtable` and `zu_codec_info` may be appended to without an
  ABI bump, and `level_fast`/`level_best` are the first use of it.**
  `zu_register_codec()` requires only the prefix the core actually
  dereferences (through `bound`, as `ZU_VTABLE_REQUIRED_SIZE`), not the
  full current `sizeof` — otherwise every appended field silently breaks
  satellite codecs compiled against an older header, which is exactly
  what `struct_size` exists to prevent. `zu_codec_get_info()` is the
  mirror image: it writes the appended fields only when the *caller’s*
  `struct_size` covers them, because the caller allocated that struct.
  Neither path is reachable from zukomp’s own suite (registration is
  init-time only, from vtables this build compiled itself), so
  `test-levels.R` drives the defaulting rules directly through
  `zu_test_vtable_levels()` and `zu_test_info_short()`.
- **A codec’s name is declared independently of its implementation.**
  `zu_int_declared[]` in `src/zu_registry.c` lists every codec this
  build knows the *name* of; the registry lists the ones with a vtable.
  That split is what lets
  [`komp_codecs()`](https://pedrobtz.github.io/zukomp/reference/komp_codecs.md)
  show a `zstd` row with `available = FALSE` instead of pretending the
...
Six libFuzzer targets in
[fuzz/](https://pedrobtz.github.io/zukomp/fuzz/) link the **pure-C core
with no R in the process**. That is why `src/zu_internal.h` must stay
free of R — R-dependent internals live in `src/zu_rglue.h`, and an
`Rinternals.h` error when building the fuzzers means something leaked
into the core.
...
Apple’s clang ships **no libFuzzer runtime**, which is what
`--standalone` is for: the same targets behind a `main()` that replays
files, so the corpus and every committed crasher stay checkable on
macOS.

Decoders fuzz under a 16 MiB output cap — without one you just
rediscover decompression bombs and report them as OOM.
`fuzz_gzip_header` drives the parser directly so every input is spent on
the riskiest code rather than on DEFLATE.

**A fuzz finding without a regression test is a finding that can come
back.** Minimise it, commit it to `fuzz/corpus/regressions/`, note it in
that directory’s README, and add a testthat test.
...
- **Self-sufficient.** Every test builds its own inputs inside the
  `test_that()` block; no file-scope objects. Repetition beats
  cleverness — a truncation failure at byte 217 should be reproducible
  from the failing test alone.
- **Self-contained.** Global state via `withr::local_*()`; randomised
  payloads via
  [`withr::local_seed()`](https://withr.r-lib.org/reference/with_seed.html).
  `setup-state.R` installs a state inspector that catches leaked options
  or stray codec registrations.
- **Assert on condition classes, never message text.**
  `expect_error(..., class = "zukomp_checksum_error")`. Wording is
  covered separately by snapshots, so a rewording is one snapshot diff
...
- **`zu_test_stream()`** — an unexported, always-compiled `.Call`
  harness driving the C stream driver at caller-chosen input/output
  chunk sizes. Chunk-boundary correctness is what `zuhttp` depends on
  and what silently rots, so it is tested from Stage 6 even though the R
  streaming API is phase 2.
- **Payload constructors must be byte-explicit, never built from
  source-file string literals.** `new_payload("utf8")` originally used a
  literal, so its bytes depended on how R decoded `helper-corpus.R`’s
  encoding — which varies by platform and locale, and made a fixture
  generated on one machine fail on another. It is spelled out byte by
  byte now. The gzip interop test is what caught it.
- **Interop uses committed fixtures, never external processes.** The
  corpus lives in `tests/testthat/fixtures/` with provenance in its
  `MANIFEST.tsv`; regenerate with `Rscript tools/make-fixtures.R` and
  verify reproducibility with `--check`. The generators source
  `helper-corpus.R` so a fixture’s payload and the
  `new_payload(kind, n)` a test compares against cannot drift. `--check`
  treats a *different generator version* producing different bytes as
  expected, and only the same version doing so as a failure. Note
  `"random"` is deliberately absent from the corpus (it depends on R’s
  RNG stream); `"lcg"` is the reproducible incompressible stand-in that
  exercises DEFLATE’s stored blocks. `tools/make-fixtures.R` runs
...
**CI runs in three tiers, and only gctorture changes between the first
two.** A push to a pull request is *quick*: `native-checks.yaml` passes
`profile: quick` to the shared `gctorture.yml`, which runs at step 500.
The push to main after merge, and any pull request with the
**`full-ci`** label, is *full*: step 20. The nightly schedules are the
third tier — `native-checks` in full again, and `fuzz.yaml` at 900 s per
target instead of the 60 s smoke run. gctorture alone takes the profile
because it alone sets how long a pull request waits (39m35s at step 20
on PR \#31, with nothing else in `native-checks` above four minutes).
Handing the profile to `sanitizers`, `valgrind` or `r-cmd-check` too
would drop ASan, valgrind and the macOS/Windows legs from pull requests
and save no wall-clock, since those finish in parallel inside the
gctorture window. The price is that a finding only step 20 makes shows
up on main after merge. A merge queue would catch it first, but
`merge_group` never fires on a personal-account repository. So put
`full-ci` on a pull request that touches `PROTECT`-sensitive R glue
(`src/zukomp_r.c`, `src/zu_whole.c`, external-pointer lifetimes), and
`rchk` still checks PROTECT discipline statically on every push. Every
PR-triggered workflow carries a `concurrency` group keyed on the PR
number, falling back to `github.run_id`: a new push cancels the run it
replaces, and no push to main is ever cancelled. The group must live in
the calling workflow, never in a reusable one, whose `github.workflow`
resolves to the caller’s.

**The coverage badge lives on `gh-pages`, at `badges/coverage.svg`**,
not on main. `coverage.yaml` passes `badge-branch: gh-pages`, so merges
no longer interleave with bot commits, and the badge keeps updating if
main ever gets a ruleset requiring pull requests. The branch is shared
with the pkgdown deploy, which is why `pkgdown.yaml`’s deploy step needs
both `clean: false` (or it deletes the badge as a file pkgdown did not
generate) and `force: false` (or it silently overwrites a badge commit
that landed between its checkout and its push).
...
`coverage.yml` is the one to read hardest, since it is the only
r-actions job holding a repository write token.
...
- **Sanitizer flags must go in `~/.R/Makevars`, and must include
  `LDFLAGS`.** `etc/Makeconf` assigns `CFLAGS`, `CXXFLAGS` *and*
  `LDFLAGS` with `=`, and make prefers a makefile assignment over the
  environment — so `CFLAGS=… R CMD INSTALL` builds uninstrumented and
  the job passes vacuously. This is not hypothetical: the shared
  `sanitizers` workflow in `pedrobtz/r-actions@v1` sets them as env
  vars, and its logs show `-fsanitize` only in the environment echo,
...
- **The two sanitizers answer different questions and run in different
  places.** UBSan runs on an ordinary runner: clang links no sanitizer
  runtime into a `-shared` output, so it needs `-shared-libsan` and an
  rpath to `clang -print-runtime-dir`, but its runtime can then arrive
  by `DT_NEEDED` at `dlopen` time. ASan cannot work that way — it must
  be first in the process, so instrumenting a `.so` that R `dlopen`s
  needs `-shared-libasan`, an `LD_PRELOAD` into R, and `detect_leaks=0`
  (R does not free on exit, so leak detection reports the interpreter).
  zukomp therefore gets ASan two ways, neither of them that:
  `fuzz/build.sh --standalone`, where the pure C core links into a real
  executable, and the shared workflow’s `asan: true`, which uses the
  R-hub containers where R itself is instrumented and so covers the R
  glue — external pointers, `PROTECT` discipline, longjmp paths — that
  no fuzz target reaches.
- **`rchk` runs with `fail-on-findings: true`, and the package is at
  zero.** It had two: unprotected `SEXP`s held live across
  `zu_encoder_free()`. That is a vtable function pointer, so rchk must
  assume it can allocate — and it is right to, since a third-party
  codec’s `encoder_free` could do anything. Free before building the
  result rather than after, and no `PROTECT` is needed. Note rchk’s own
  abstraction-limit lines (`too many states … in function bcEval_loop`)
  are R’s functions, not yours, and must not be matched as findings;
  real ones carry `[UP]`/`[PB]`/`[UPB]` and name a line in this package.
